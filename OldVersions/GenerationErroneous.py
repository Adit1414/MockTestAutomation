...
MODEL = 'gpt-4-turbo'
EXCEL_PATH = "D:\\aditya data\\Freelance Projects\\Automation\\CS\\Syllabus.xlsx"
CHUNK_SIZE = 5
TESTING = True
...
#             return "\n".join([f"Question {i+1} - Correct" for i in range(CHUNK_SIZE)])
#         else:
#             return "\n\n".join([
#                 f"Q{i+1}. Sample question on Topic.\nAnswer: {chr(65 + i%4)}\nExplanation: Because XYZ."
...
#                     raise RuntimeError("❌ GPT call failed after multiple attempts.")
#                 time.sleep(2)
...
def check_file_accessibility(filepath):
    try:
        ...
        raise RuntimeError(f"❌ Cannot access file '{filepath}': {e}")
...
if __name__ == "__main__":
    ...
    try:
        check_file_access("D:\\aditya data\\Freelance Projects\\Automation\\CS\\Questions.docx")
        check_file_access("D:\\aditya data\\Freelance Projects\\Automation\\CS\\Verifications.docx")
    except Exception as e:
        print(e)
        exit(1)
    all_questions = []
    ...
    for qtype, prompt in generated_prompts:
        ...
        questions_split = textwrap.dedent(generated_chunk).strip().split("\n\n")
    ...
    save_to_docx(combined_questions_text, "D:\\aditya data\\Freelance Projects\\Automation\\CS\\Questions.docx")

    verified_output = ""
    for i in range(0, len(all_questions), CHUNK_SIZE):
        chunk = all_questions[i:i+CHUNK_SIZE]
        if len(chunk) < CHUNK_SIZE:
            print(f"⚠️ Skipping incomplete chunk at end: {chunk}")
            continue
        combined_text = "\n\n".join(chunk)

        # Verify the chunk via GPT
        verification = call_gpt(verify_prompt(combined_text))
        verified_output += verification + "\n\n"

    save_to_docx(verified_output.strip(), "D:\\aditya data\\Freelance Projects\\Automation\\CS\\Verifications.docx")
